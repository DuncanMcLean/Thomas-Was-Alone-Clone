...
int** LevelManager::nextLevel(VertexArray& rVaLevel)
{
	...
	case 4:
		...
		m_StartPosition.x = 55;
		m_StartPosition.y = 235;
		m_BaseTimeLimit = 50.0f;
		break;

	case 5:
		levelToLoad = "levels/level5.txt";
		m_StartPosition.x = 270;
	...
	case 8:
		...
		m_StartPosition.x = 1350;
		m_StartPosition.y = 40;
		m_BaseTimeLimit = 20.0f;
		break;
	...
	for (int x = 0; x < m_LevelSize.x; x++)
	{
		for (int y = 0; y < m_LevelSize.y; y++)
		{
			...
			rVaLevel[currentVertex + 4].position =
				Vector2f((x * TILE_SIZE), (y * TILE_SIZE) + TILE_SIZE);

			rVaLevel[currentVertex + 5].position =
				Vector2f((x * TILE_SIZE), (y * TILE_SIZE) + TILE_SIZE);

			rVaLevel[currentVertex + 6].position =
				Vector2f((x * TILE_SIZE), (y * TILE_SIZE) + TILE_SIZE);

			rVaLevel[currentVertex + 7].position =
				Vector2f((x * TILE_SIZE), (y * TILE_SIZE) + TILE_SIZE);
			...
			rVaLevel[currentVertex + 4].texCoords =
				Vector2f(0, TILE_SIZE + verticalOffset);

			rVaLevel[currentVertex + 5].texCoords =
				Vector2f(0, TILE_SIZE + verticalOffset);

			rVaLevel[currentVertex + 6].texCoords =
				Vector2f(0, TILE_SIZE + verticalOffset);

			rVaLevel[currentVertex + 7].texCoords =
				Vector2f(0, TILE_SIZE + verticalOffset);
